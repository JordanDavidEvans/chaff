...
function startProcessing() {
  if (!processing) {
    ...
    chrome.tabs.create({ active: true, index: 0, pinned: true }, tab => {
...
function beginBrowsing() {
  ...
  BROWSING_PARAMETERS["siteDepth"] = randInt(
    Math.floor(settings.get("maxSiteDepth") * settings.get("siteDepthVariance")),
    settings.get("maxSiteDepth")
  );
...
function startBrowseTimeout() {
  ...
  browsingTimeout = setTimeout(
    browseError,
    randInt(
      Math.floor(settings.get("browsingTimeoutMax") * settings.get("browsingTimeoutVariance")),
      settings.get("browsingTimeoutMax")
    ) * 1000
  );
}

function browseError() {
  let browseTimeoutNewURLtmp = historyURLs.pop(); // This one is the bad URL
  browseTimeoutNewURLtmp = historyURLs.pop();

  if (browseTimeoutNewURLtmp == browseTimeoutNewURL) {
    console.log(
      "!!Browse Timeout: Timed out from same URL last time. Backing up one more. [" +
        browseTimeoutNewURLtmp +
        "] == [" +
        browseTimeoutNewURL +
        "]"
    );
...
function analyzeHistory(currentURL) {
  ...
  if (historyURLs.length > BROWSING_PARAMETERS["siteDepth"]) {
    const tld = getDomain(currentURL);
    let count = 0;
    console.log(
      "Analyzing History [Staleness]: Domain [" + tld + "], Depth [" + BROWSING_PARAMETERS["siteDepth"] + "]"
    );
    for (let i = historyURLs.length - 1; i >= 0; i--) {
      const domain = getDomain(historyURLs[i]);
      if (domain == tld) {
        count++;
      } else {
        break;
...
function setIdleTime() {
  ...
  let idleTime = settings.get("idleTime");
  idleTime *= 60;
